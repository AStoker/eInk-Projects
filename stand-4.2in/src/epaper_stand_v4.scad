...
bat_w = 34.0; bat_h = 62.0; bat_t = 5.0;       // 1200 mAh LiPo, Adafruit 258
bat_clr = 0.4; bat_fence = 1.0;
chg_w = 25.4; chg_h = 20.3;                    // bq24074 breakout
conn_h = 6.0;    // assumed height of the module's 8-pin header off its back
usb_w = 13.0;    // port opening width
btn_n = 0;       // side buttons: 0 = none, 3 = three on the +X edge
btn_d = 4.2; btn_sp = 12.0;

/* [Body] */
depth = 23.0; front_t = 1.4; cover_t = 1.4; pk_floor_t = 1.8;
wall = 2.2; white_show = 1.3;
corner_r = 5.0; cav_r = 2.0; front_chf = 0.8; win_chf = 0.9; rear_chf = 2.0;
...
// -X column: the cell, directly opposite the module's ribbon/connector edge,
//            so the module's 8-pin header has the full depth over it
bat_x0 = cav_x0 + 0.5;
bat_cx = bat_x0 + bat_w/2;  bat_cz = Zc;
// +X column: Perma-Proto low, Waveshare driver board above it
proto_x1 = cav_x1 - 0.3;  proto_x0 = proto_x1 - proto_w;
proto_z0 = cav_z0 + 1.0;
proto_cx = proto_x0 + proto_w/2;  proto_cz = proto_z0 + proto_h/2;
proto_back = cov_in_pk - 0.5;                     // sits on the puck face
proto_face = proto_back - proto_t;
drv_x1 = cav_x1 - 3.2;  drv_x0 = drv_x1 - drv_w;
...
module usb_cut(){
    // flash port (Waveshare board) through the +Z wall
    translate([flash_x-usb_w/2, flash_y0, cav_z1-1.2])
        cube([usb_w, flash_y1-flash_y0, H-cav_z1+2]);
...
module rear_chamfer(){
    difference(){
        translate([-W, depth-rear_chf-0.01, -H]) cube([2*W, rear_chf+0.02, 3*H]);
        hull(){ prism(W, H, corner_r, 0.01, 0, Zc, depth-rear_chf-0.005);
                prism(W-2*rear_chf, H-2*rear_chf, max(corner_r-rear_chf,0.5),
                      0.01, 0, Zc, depth-0.005); } } }
...
module cover(){
    difference(){
        union(){
            ...
            for(sz=[-1,1])
                translate([bat_cx, mod_back+rib_gap, Zc+sz*(cav_h/2-rib_inset-rib_pad/2)])
                    xzext(cov_in-mod_back-rib_gap) square([bat_w-2, rib_pad],center=true);
            ...
            for(sx=[-1,1])
                translate([drv_cx + sx*(drv_w/2+drv_clr+0.3), drv_back-2.6, drv_cz])
                    xzext(cov_in-drv_back+2.6) square([3.0, drv_h-1.0],center=true);
            ...
            translate([(cav_x0+0.3 + bat_cx+bat_w/2+bat_clr+bat_fence)/2, cov_in_pk, bat_cz])
                xzext(cov_in-cov_in_pk+0.01)
                    square([bat_cx+bat_w/2+bat_clr+bat_fence-(cav_x0+0.3),
                            bat_h+2*bat_clr+2*bat_fence],center=true);
            translate([bat_cx+bat_w/2+bat_clr+bat_fence/2, cov_in_pk-bat_t, bat_cz])
                xzext(bat_t) square([bat_fence, bat_h-14],center=true);
            for(sz=[-1,1])
                translate([bat_cx, cov_in_pk-bat_t, bat_cz+sz*(bat_h/2+bat_clr+bat_fence/2)])
                    xzext(bat_t) square([bat_w-10, bat_fence],center=true);
            // ---- guide rails for the rear USB-C breakout board
            for(sz=[-1,1])
                translate([ucb_x, cov_in-ucb_l, ucb_z+sz*(ucb_t/2+0.15+1.0)])
                    xzext(ucb_l-5.0) square([ucb_w+4, 2.0],center=true);
...
module mock_board(){
    ...
    color("#155799") translate([proto_x1-chg_w/2-1, proto_face-2.5, chg_z])
        xzext(1.6) square([chg_w, chg_h],center=true);
